...
async function showServiceConfigModal(serviceName) {
  try {
    ...
    if (isDemoMode) {
      ...
      configReq = getDemoServiceConfig(serviceName);
    } else {
      const response = await fetch(`/api/services/${serviceName}/config`);
      configReq = await response.json();
    }
...
async function handleCreateRoute(e) {
  ...
  if (routeData.source === routeData.target) {
    alert('Source and target services cannot be the same');
    return;
  }
  ...
  try {
    if (isDemoMode) {
      ...
      updateRoutesUI();
      closeModal();
      alert('Demo: Route created successfully!');
    } else {
    ...
    <div class="glass rounded-lg p-4 border border-gray-600">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          <span class="text-blue-400 font-medium">${route.source}</span>
          <span class="text-gray-400">→</span>
          <span class="text-green-400 font-medium">${route.target}</span>
          ${route.filter ? `<span class="text-xs bg-orange-500 px-2 py-1 rounded">Filter: ${route.filter}</span>` : ''}
        </div>
...
async function deleteRoute(routeId) {
  ...
  try {
    if (isDemoMode) {
      ...
      updateRoutesUI();
      alert('Demo: Route deleted successfully!');
    } else {
